...
function startGame() {
    ...
    playerScoreLabel.textContent = `${playerSymbol.toUpperCase()} (تو)`;
    cpuScoreLabel.textContent = `${cpuSymbol.toUpperCase()} (ربات)`;
    resetBoard();
}
...
function resetBoard() {
    ...
    cells.forEach(cell => {
        ...
        cell.innerHTML = `<i></i>`;
        cell.classList.add(`${playerSymbol}-hover`);
        cell.setAttribute('aria-label', 'خانهٔ خالی');
...
function makeMove(index, symbol) {
    ...
    cell.setAttribute('aria-label', `خانه با ${symbol.toUpperCase()} پر شد`);
...
function showResult(isDraw, winner) {
    if (isDraw) {
        ...
        modalWinnerAnnouncement.innerHTML = `<h2 class="tie-color">این دور مساوی شد</h2>`;
    } else {
        const winnerIsPlayer = winner === playerSymbol;
        winnerIsPlayer ? scores.player++ : scores.cpu++;
        modalResultText.textContent = winnerIsPlayer ? 'بردی!' : 'این دور را ربات برد.';
        modalWinnerAnnouncement.innerHTML = `
            <i class="icon-winner bi bi-${winner === 'x' ? 'x-lg' : 'circle'}"></i>
            <h2 class="${winner}-win-color">برندهٔ این دور</h2>`;
