...
function App() {
  ...
  const renderPage = () => {
    ...
    return <HomePage name={playerName} activeRooms={activeRooms} onPractice={startPractice} onFriendsGame={openFriendsGame} onOnlineMafia={() => setPage('mafia-setup')} onResumeRoom={resumeActiveRoom} onGames={() => setPage('games')} started={sessionProgress.started} earnedMedals={sessionProgress.earnedMedals} />
  }
...
function HomePage({ name, activeRooms, onPractice, onFriendsGame, onOnlineMafia, onResumeRoom, onGames, started, earnedMedals }: { name: string; activeRooms: ActiveRoomSummary[]; onPractice: (game: GameId) => void; onFriendsGame: (game: GameId) => void; onOnlineMafia: () => void; onResumeRoom: (room: ActiveRoomSummary) => void; onGames: () => void; started: ReturnType<typeof useSessionPlayProgress>['started']; earnedMedals: SessionMedal[] }) {
  const { language, t, format } = useLanguage()
  const featuredGames = games
    .filter((game) => game.id === 'mafia' || game.id === 'tic-tac-toe' || game.id === 'uno')
    .map((game) => localizeGame(game, language))
