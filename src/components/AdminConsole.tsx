...
import {
  Activity, AlertTriangle, ArrowLeft, Ban, BarChart3, ChevronLeft, CircleStop, Clock3, Copy, Database, Gamepad2, ImagePlus,
...
import AdminAvatarLibrary from './AdminAvatarLibrary'
...
type ConsoleTab = 'overview' | 'users' | 'launch' | 'rooms' | 'identity'
...
export default function AdminConsole({ onBack, onOpenPractice, onEnterRoom, notify }: AdminConsoleProps) {
  ...
  const copy = fa ? {
    ...
    refresh: 'به‌روزرسانی', overview: 'نبض پلتفرم', users: 'کاربران و ایمنی', launch: 'عملیات زنده', rooms: 'اتاق‌های من', identity: 'هویت و آواتارها',
    ...
    refresh: 'Refresh', overview: 'Platform pulse', users: 'People & safety', launch: 'Live operations', rooms: 'My rooms', identity: 'Identity & avatars',
  ...
  return <section className="admin-console" dir={fa ? 'rtl' : 'ltr'}>
    ...
    <div className="admin-console-layout">
      <nav className="admin-console-nav" aria-label={copy.eyebrow}>
        {([
          ['overview', BarChart3, copy.overview], ['launch', Rocket, copy.launch], ['rooms', Database, copy.rooms], ['users', UsersRound, copy.users], ['identity', ImagePlus, copy.identity],
